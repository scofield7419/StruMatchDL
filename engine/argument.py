...
# DATA
    parser.add_argument(
            "--dataset", default="ParaNMT", nargs=nargs,
            choices=["ParaNMT", "QUORA", "WMT14(EN-DE)", "WMT14(EN-FR)"],
            help='the dataset would be used [ParaNMT]')
    parser.add_argument(
            '--fold_attr', type=int, default=1,
            nargs=nargs, choices=range(0, 2),
            help='fold the attribute into one token or not \
                    (only activate when using ParaNMT dataset) [1]')
    parser.add_argument(
    ...
    parser.add_argument(
            ...
            nargs=nargs, choices=range(0, 2),
            help='use Fasttext embeddings or not [0]')
    parser.add_argument(
...
# PARAM
    parser.add_argument(
            '--lmbda1', type=float, default=0.5, nargs=nargs,
            help='lmbda1 [0.5]')
    parser.add_argument(
            '--lmbda2', type=float, default=0.6, nargs=nargs,
            help='lmbda2 [0.6]')
    parser.add_argument(
            '--lmbda3', type=float, default=0.2, nargs=nargs,
            help='lmbda3 [0.2]')
    parser.add_argument(
            '--threshold_omega', type=float, default=0.3, nargs=nargs,
            help='threshold_omega [0.3]')
    parser.add_argument(
            '--threshold_sigma', type=float, default=0.5, nargs=nargs,
            help='threshold_sigma [0.5]')
# TRAINING
    parser.add_argument(
    ...
    parser.add_argument(
            '--lm2_model_dir', type=str, default=None,
            help='Language model2 dir'
    )
    parser.add_argument(
